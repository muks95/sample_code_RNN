...
class Tree:
    ...
    def parse(self, tokens, parent=None):
        ...
        if( node.left.dropout == True and node.right.dropout == True):
            node.dropout=True
        return node
...
def simplified_data(num_train, num_dev, num_test):
    ...
    pos_trees_len=len(pos_trees)
    pos_trees_sample=[]
    for i in range(pos_trees_len):
        tree_len=len(pos_trees[i].get_words())
        if(tree_len > 3):
            pos_trees_sample.append(pos_trees[i])
            if(len(pos_trees_sample)==2):
                break

    neg_trees_len=len(neg_trees)
    neg_trees_sample=[]
    for i in range(neg_trees_len):
        tree_len=len(neg_trees[i].get_words())
        if(tree_len > 3):
            neg_trees_sample.append(neg_trees[i])
            if(len(neg_trees_sample)==2):
                break



    return train + pos_trees_sample+neg_trees_sample, dev, test , pos_trees_sample+neg_trees_sample

def sample_data():
    trees=loadTrees('train') + loadTrees('dev') + loadTrees('test')
    pos_trees = [t for t in trees if t.root.label==4]
    neg_trees = [t for t in trees if t.root.label==0]
    return pos_trees[:5] + neg_trees[:5]
